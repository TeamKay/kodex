...
export function LessonItem({lesson, slug, isActive, completed}: iAppProps){
    ...
    return (
        <Link href={`/dashboard/student/enrolled/${slug}/${lesson.id}`} 
        className={buttonVariants({variant: completed ? 'secondary' : 'outline', className: cn(
            "w-full p-2.5 h-auto justify-start transition-all", completed && "bg-green-100 dark:bg-green-900/30 border-green-300 dark:border-green-700 hover:bg-green-200 dark:hover:bg-green-900/50 text-green-800 dark:text-green-200",
