...
import { reorderChapters, reorderLessons } from "../actions/edit-course";
...
export default function CourseStructure({data}: iAppProps){
   ...
   function handleDragEnd(event: DragEndEvent) {
    ...
    if(activeType === "chapter"){
        ...
        if(oldIndex === -1 || newIndex === -1){
            toast.error("Could not find chapter old/new idex for reordering");
            return;
        }
  ...
  function toggleChapter(chapterId: string) {
    setItems(
        items.map((chapter) => 
            chapterId === chapterId 
                ? {...chapter, isOpen: !chapter.isOpen } : chapter
            )
        );
    }
                                                                            ...
                                                                            {(lessonListeners) => (
                                                                                <div className="flex items-center justify-between p-2 hover:bg-accent rounded-sm">
                                                                                    <div className="flex items-center gap-2">
                                                                                        ...
                                                                                        <Link href={`/dashboard/educator/courses/${data.id}/${item.id}/${lesson.id}`}>{lesson.title}</Link>
                                                                                    </div>
